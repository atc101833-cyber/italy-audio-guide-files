...
const V="italy-guide-v1",CORE=V+"-core",PREFIX=V+"-city-";
const CORE_ASSETS=["./", "./index.html", "./offline.html", "./manifest.json", "./cover.jpg", "./italy-pattern-background.jpg", "./player-skin-clean.png", "./milan.jpg", "./venice.jpg", "./florence.jpg", "./rome.jpg"];
...
self.addEventListener("activate",e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith("italy-guide-")&&!k.startsWith(V)).map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
self.addEventListener("message",e=>{
 const d=e.data||{},p=e.ports&&e.ports[0];
 if(d.type!=="CACHE_CITY"||!CITY_ASSETS[d.city])return;
 e.waitUntil((async()=>{
  try{
   const arr=CITY_ASSETS[d.city],cache=await caches.open(PREFIX+d.city); let done=0;
   for(const a of arr){const r=await fetch(new Request(a,{cache:"reload"})); if(!r.ok)throw new Error(a); await cache.put(a,r.clone()); done++; if(p)p.postMessage({type:"CACHE_PROGRESS",done,total:arr.length});}
   if(p)p.postMessage({type:"CACHE_COMPLETE"});
  }catch(err){if(p)p.postMessage({type:"CACHE_ERROR",message:String(err)});}
 })());
});
self.addEventListener("fetch",e=>{
 if(e.request.method!=="GET")return;
 const u=new URL(e.request.url); if(u.origin!==location.origin)return;
 e.respondWith(caches.match(e.request).then(c=>c||fetch(e.request).then(r=>{if(r&&r.status===200)caches.open(CORE).then(k=>k.put(e.request,r.clone())); return r;}).catch(()=>e.request.mode==="navigate"?caches.match("./offline.html"):undefined)));
});
