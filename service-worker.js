...
const V="italy-guide-v2",CORE=V+"-core",PREFIX=V+"-city-";
...
self.addEventListener("message", e => {
  ...
  if (d.type === "GET_CACHE_STATUS") {
    e.waitUntil((async () => {
      ...
      const cities = Object.keys(CITY_ASSETS).filter(city => keys.includes(PREFIX + city));
      if (p) p.postMessage({ type: "CACHE_STATUS", cities });
    })());
    return;
  }
  ...
  e.waitUntil((async () => {
    try {
      ...
      for (const a of arr) {
        ...
        if (p) p.postMessage({ type: "CACHE_PROGRESS", done, total: arr.length });
      }
      if (p) p.postMessage({ type: "CACHE_COMPLETE", city: d.city });
    } catch (error) {
      if (p) p.postMessage({ type: "CACHE_ERROR", message: String(error) });
    }
  })());
